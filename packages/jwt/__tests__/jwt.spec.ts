...
it('should sign and verify a jwt with RS256', async () => {
	...
	const licenseV3 = {
		...
		limits: {
			activeUsers: [
				{ max: 300, behavior: 'start_fair_policy' },
				{ max: 30000, behavior: 'prevent_action' },
				{ max: 31000, behavior: 'invalidate_license' },
			],
			guestUsers: [
				{ max: 4300, behavior: 'start_fair_policy' },
				{ max: 4000, behavior: 'prevent_action' },
				{ max: 5000, behavior: 'invalidate_license' },
			],
			roomsPerGuest: [
				{ max: 50, behavior: 'start_fair_policy' },
				{ max: 100, behavior: 'prevent_action' },
			],
			privateApps: [
				{ max: 50, behavior: 'start_fair_policy' },
				{ max: 100, behavior: 'prevent_action' },
				{ max: 110, behavior: 'invalidate_license' },
			],
			marketplaceApps: [
				{ max: 50, behavior: 'start_fair_policy' },
				{ max: 100, behavior: 'prevent_action' },
